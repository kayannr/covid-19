---
title: "Covid-19 Analysis: Effectiveness of Covid-19 Policies"
author: "Kay Royo"
date: "3/13/22"
output: 
...
#load WHO data
who_data <-read.csv("C:/Users/kayan/UCD/STA141A/covid-19/covid19-a/Data/covid/WHO-COVID-19-global-data.csv")
...
face_data <-read.csv("C:/Users/kayan/UCD/STA141A/covid-19/covid19-a/Data/policies/face-covering-policies-covid.csv", header=T)
...
vacc_data <-read.csv("C:/Users/kayan/UCD/STA141A/covid-19/covid19-a/Data/policies/covid-vaccination-policy.csv", header=T)
...
internal_data <-read.csv("C:/Users/kayan/UCD/STA141A/covid-19/covid19-a/Data/policies/internal-movement-covid.csv", header=T)
...
stayhome_data <-read.csv("C:/Users/kayan/UCD/STA141A/covid-19/covid19-a/Data/policies/stay-at-home-covid.csv", header=T)
...
inttravel_data <-read.csv("C:/Users/kayan/UCD/STA141A/covid-19/covid19-a/Data/policies/international-travel-covid.csv", header=T)
...
To visualize the proportion of variances explained by each MCA dimension, the function _fviz_screeplot()_ is used to generate the Scree plot shown in Figure 5 below. This Scree plot can be interpreted in a similar way as the PCA Scree plots for principal components. The scree plot below does not appear to have any significant drop or "elbow". Therefore, it can be inferred that there are no dominating directions that exist in the data. Hence, all the dimensions can be retained for this analysis. 
...
<span style='color:black'>
Since the monthly dataset contains various observations where the variable _New_cases_ is zero, these observations can be excluded in order to avoid encountering issues when using log transformation. Figure 9 below shows that the relative frequency of 0 new cases in the monthly data is dominant. The histogram of the log of _New_cases_ below in Figure 9 shows that the distribution of the variable _New_cases_ is normally distributed after performing the log transformation. Thus, the ANOVA model can be fitted using the log of _New_cases_ with nonzero values. After the removal of rows with 0 new cases recorded, the total number of observations in the new  simplified dataset decreased from 4641 to 4164, which is not a significant difference. 
</span>

```{r, echo = FALSE,  fig.align = 'left', fig.cap = 'Figure 9: Distribution of New_cases',message=FALSE, warning=FALSE}
...
# print(nrow(data_monthly1))
# print(nrow(data_monthly2))
# print(nrow(data_monthly2_f))
```
...
<span style='color:black'>
The full model fitted with the monthly dataset excluding observations where _New_cases_ is 0 and the log of _New_cases_ as the response variable does not severely violate the ANOVA model assumptions as shown in its diagnostic plot below (Figure 10). Additionally, The Box-Cox plot in Figure 11 shows that fitting the full model with log of _New_cases_ as the response does not need any further transformation since $\lambda$ is approximately 1. 
</span>

```{r, echo = FALSE,  fig.align = 'left', fig.cap = 'Figure 10: Box-Cox Plot',message=FALSE, warning=FALSE}
...
<span style='color:black'>
It can be further tested whether the interactions between the categorical predictors have a significant impact on the response variable _New_cases_. Based on the ANOVA table displayed below, the interaction between most of these factors appear to have a significant impact on the response with small p-values suggesting that there there is a strong evidence against the null hypothesis that the interactions between the predictors have no significant effect on the response. Additional tests can be performed in order to find the best combination of predictors for this case where their effect on the response is statistically significant. The summary output of the full ANOVA model shown below also shows that all 5 policy factors are statistically significant at significance level $\alpha$=0.001. By looking at this summary output, we can see that the interaction between most of the factors are statistically significant with the exception of the interaction between _Stay_home_ and _International_travel_. Therefore, additional tests are not needed to identify which of the interactions are the best combinations. Based on these results, most of the interactions between the categorical predictors are statistically significant. Therefore the final anova model must be additive with most of the possible interactions. 
</span>
...
```{r, echo = FALSE, message=FALSE, warning=FALSE}
# full model with significant interactions 
full_mod=aov(New_cases~Face_cover+Vaccination+Internal_movement+Stay_home+International_travel+WHO_region+Face_cover:Vaccination+Face_cover:Internal_movement+Face_cover:Stay_home+Face_cover:International_travel+Face_cover:WHO_region +Vaccination:Internal_movement+Vaccination:Stay_home+Vaccination:International_travel+Vaccination:WHO_region+Internal_movement:Stay_home+Internal_movement:International_travel+Internal_movement:WHO_region + Stay_home:WHO_region +International_travel:WHO_region , data=data_monthly2_f)
...
```
<span style='color:black'>
The ANOVA table of the full additive ANOVA model with significant interactions is displayed below and shows that all of the factors and some of their interactions are statistically significant at significance level $\alpha=0.05$, which means that they have a significant effect on the number of covid-19 cases recorded. Based on the effect size (`ges`), the factors _Face_cover_, _Vaccination_, and _WHO_region_ continue to show the highest effect size out of all the 6 factors included in this model. Meanwhile, the interactions with the largest effect size are  _International_travel:WHO_region_, _Face_cover:WHO_region_, and _Vaccination:WHO_region_. It is evident that the interaction between the 5 policies and the region tend to have large effect size. 
</span>

ANOVA Table (Type II test) for full additive ANOVA model with significant interactions:
```{r, echo = FALSE, message=FALSE, warning=FALSE}
library(rstatix)
res.aov <- anova_test(New_cases~Face_cover+Vaccination+Internal_movement+Stay_home+International_travel+WHO_region+Face_cover:Vaccination+Face_cover:Internal_movement+Face_cover:Stay_home+Face_cover:International_travel+Face_cover:WHO_region +Vaccination:Internal_movement+Vaccination:Stay_home+Vaccination:International_travel+Vaccination:WHO_region+Internal_movement:Stay_home+Internal_movement:International_travel+Internal_movement:WHO_region + Stay_home:WHO_region +International_travel:WHO_region, data=data_monthly2_f)
res.aov
```
...
<span style='color:black'>
In _Figure 12_ below, it is evident that the means within each factor vary between different levels or groups. Tukey's HSD (honestly significant difference) is utilized to perform the pairwise comparison of means ($\mu_i -\mu_{i'}$). The coverage is exactly 1−$\alpha$ for this study it is at least 1−α because it is an unbalanced case.The following tables show the mean pairwise comparisons within each factor using the Tukey HSD method. The p-values in each table appears to be extremely small the pairwise differences are significant, which proves the observation obtained using _Figure 12_ in the previous section. 
</span>

```{r, echo= FALSE,  fig.align = 'left', fig.cap = 'Figure 12: Main Effect Plot (95% Confidence Interval)',  message=FALSE, warning=FALSE}
...
p0 <- plotmeans(New_cases~WHO_region,data= data_monthly2_f, xlab="WHO_region",ylab="Covid-19 cases", p=.95, main="Main effect Plot with 95% CI",cex=2, pch=20, barwidth=2, barcol="blue",  bars=TRUE, ccol="red", col="red")
grid()

p1 <- plotmeans(New_cases~Face_cover,data= data_monthly2_f, xlab="Face_cover",ylab="Covid-19 cases", p=.95, main="Main effect Plot with 95% CI",cex=2, pch=20, barwidth=2, barcol="blue",  bars=TRUE, ccol="red", col="red")
grid()
# 
p2 <- plotmeans(New_cases~Vaccination,data= data_monthly2, xlab="Vaccination",ylab="Covid-19 cases", p=.95, main="Main effect Plot with 95% CI",cex=2, pch=20, barwidth=2, barcol="blue",  bars=TRUE, ccol="red", col="red")
grid()
# 
p3 <- plotmeans(New_cases~Internal_movement,data= data_monthly2, xlab="Internal movement",ylab="Covid-19 cases", p=.95, main="Main effect Plot with 95% CI",cex=2, pch=20, barwidth=2, barcol="blue",  bars=TRUE, ccol="red", col="red")
grid()
# 
# 
p4 <- plotmeans(New_cases~Stay_home,data= data_monthly2, xlab="Stay home",ylab="Covid-19 cases", p=.95, main="Main effect Plot with 95% CI",cex=2, pch=20, barwidth=2, barcol="blue",  bars=TRUE, ccol="red", col="red")
grid()
# 
p5 <- plotmeans(New_cases~International_travel,data= data_monthly2, xlab="International travel",ylab="Covid-19 cases", p=.95, main="Main effect Plot with 95% CI",cex=2, pch=20, barwidth=2, barcol="blue",  bars=TRUE, ccol="red", col="red")
grid()
```
...
idxx=list();
idxx[[1]]=data_monthly2_f$Face_cover
means.combb=tapply(data_monthly2_f$New_cases, INDEX=idxx,mean)
...
```{r, echo = FALSE, , message=FALSE, warning=FALSE}
library(car)
leveneTest(New_cases~Face_cover,data_monthly2_f)
```

Vaccination:
```{r, echo = FALSE, , message=FALSE, warning=FALSE}
leveneTest(New_cases~Vaccination,data_monthly2_f)
```

Internal movement:
```{r, echo = FALSE, , message=FALSE, warning=FALSE}
leveneTest(New_cases~Internal_movement,data_monthly2_f)
```

Stay-at-home: 
```{r, echo = FALSE, , message=FALSE, warning=FALSE}
leveneTest(New_cases~Stay_home,data_monthly2_f)
```

International travel:
```{r, echo = FALSE, , message=FALSE, warning=FALSE}
leveneTest(New_cases~International_travel,data_monthly2_f)
```

WHO region:
```{r, echo = FALSE, , message=FALSE, warning=FALSE}
leveneTest(New_cases~WHO_region,data_monthly2_f)
```
